      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="border-t-2 border-t-chart-4">
          <CardHeader className="pb-2">
            ...
            <CardTitle className="text-3xl font-bold tabular-nums">{insiderScans.length}</CardTitle>
          </CardHeader>
        </Card>
        <Card className="border-t-2 border-t-chart-1">
          <CardHeader className="pb-2">
            <CardDescription>Market Notes</CardDescription>
            <CardTitle className="text-3xl font-bold tabular-nums">{marketNotes.length}</CardTitle>
          </CardHeader>
        </Card>
        <Card className="border-t-2 border-t-chart-2">
          <CardHeader className="pb-2">
            <CardDescription>Macro Analysis</CardDescription>
            <CardTitle className="text-3xl font-bold tabular-nums">{macroNotes.length}</CardTitle>
          </CardHeader>
        </Card>
      </div>
        ...
        <CardContent>
          <div className="space-y-3">
            {insiderScans.slice(0, 10).map((n) => (
              <div key={n.path} className="p-3 rounded-lg border border-border">
        ...
        <CardContent>
          <div className="space-y-2">
            {macroNotes.slice(0, 10).map((n) => (
              <div key={n.path} className="flex items-center justify-between py-2 border-b border-border last:border-0">
          ...
          <CardContent>
            <div className="space-y-2">
              {[...marketNotes, ...otherFinance].slice(0, 10).map((n) => (
                <div key={n.path} className="flex items-center justify-between py-1.5 border-b border-border last:border-0">
